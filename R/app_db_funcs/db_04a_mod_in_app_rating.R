...
get_project_evaluation <- function(coc_version_id, project_id) {
  get_db_query(
    "SELECT p.coc_version_id, pe.project_id, method, met_hud_thresholds, met_coc_thresholds, weighted_score, pe.version_id 
...
get_project_evaluation_scores <- function() {
  get_db_query(
    "SELECT DISTINCT project_id FROM project_evaluations WHERE weighted_score > 0"
  )
}