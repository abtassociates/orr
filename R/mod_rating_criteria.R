...
#' @param id The module's unique ID.
#' @param coc_instance_id coc_instance_id to capture user-selected version of the ORR
#' @noRd
mod_rating_criteria_server <- function(id, coc_instance_id) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns
    
    # Call sub-modules for each tab
    mod_coc_thresholds_server("coc_thresholds", coc_instance_id)
...
#' @noRd
mod_coc_thresholds_server <- function(id, coc_instance_id, all_thresholds) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns
    ...
    observe({
      ...
      selected_data <- get_db_query(selected_q, params = list(coc_instance_id()))
    ...
    observeEvent(input$save_thresholds, {
      ...
      tryCatch({
        ...
        if (length(to_remove) > 0) {
          ...
          dbExecute(DB_CON, remove_q, params = list(coc_instance_id(), to_remove))
        }
